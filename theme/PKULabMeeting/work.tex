%-*-coding:utf-8-*-
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Name:   Work Report                                                        %%
%% Author: Wolfson                                                            %%
%% Date:   2016-12-12                                                         %%
%% Discription:                                                               %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\documentclass[svgnames,table]{beamer}

%%----Packages
%%%%----Font
\usepackage{fontspec}
% used to set font for the latex document.

% using CJK to manage Chinese, need fontsec.
\setmainfont{Times New Roman}
\setsansfont{Arial}
\setmonofont{Courier New}
% \setCJKmainfont[BoldFont={方正黑体简体}, ItalicFont={方正楷体简体}]{方正大标宋简体}
% \setCJKsansfont{方正黑体简体}
% \setCJKmonofont{方正中等线简体}

%%%%----Symbol
\usepackage{textcomp}    % provide symbols.
%\usepackage{mathabx}     % more symbols like astronomy.

%%%%----Graph
\usepackage{graphicx}
\graphicspath{{pic/}}
\usepackage{tikz}

%%%%----Reference
\usepackage{hyperref}
\hypersetup{
    bookmarks=true,         % show bookmarks bar?
    unicode=true,          % non-Latin characters in Acrobat’s bookmarks
    pdftoolbar=true,        % show Acrobat’s toolbar?
    pdfmenubar=true,        % show Acrobat’s menu?
    pdffitwindow=false,     % window fit to page when opened
    pdfstartview={FitH},    % fits the width of the page to the window
    pdftitle={工作汇报},    % title
    pdfauthor={刘志恒},     % author
    pdfcreator={Wolfson},   % creator of the document
    pdfproducer={Wolfson}, % producer of the document
    pdfnewwindow=true,      % links in new PDF window
    colorlinks=true,       % false: boxed links; true: colored links
    linkcolor=black,          % color of internal links (change box color with linkbordercolor)
    citecolor=black,        % color of links to bibliography
    filecolor=black,      % color of file links
    urlcolor=black           % color of external links
}

%%----Theme
\usetheme{PKULabMeeting}

%%----Metadata
\title{Hub Screening Work Report}
\author{Zhiheng Liu}
\institute{Peking University}    % provide the information of institute.
\supervisor{Prof. Wensheng Wei}
\date{\today}
\logo{\includegraphics[height=0.06\textheight]{pku.png}}
\titlepagelogo{\includegraphics[width=0.3\textwidth]{pku.png}}
\titlepagebackground{\includegraphics[height=0.7\textwidth]{gate.jpg}}
\department{School of Life Sciences}
\backgroundlogo{\includegraphics[height=0.4\textwidth]{cloud.png}}


%%----Document
\begin{document}

\input{frames}
 
\end{document}

%%----End
